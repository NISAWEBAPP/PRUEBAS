...
map.getView().fit([-6641634.620677, -3076609.723197, -5692562.400384, -2628096.561027], map.getSize());
...
const geolocation = new ol.Geolocation({
    ...
    trackingOptions: {
        enableHighAccuracy: true, // Forces GPS use for better precision
        maximumAge: 0,            // Prevents using old/cached positions
...
geolocation.on('change:position', function () {
    ...
    // CONTINUOUS TRACKING: Auto-center map when position changes
    if (isTracking && coords) {
        map.getView().setCenter(coords);
    }
});

// Set tracking to true so it stays ready in the background
...
function handleGeolocate() {
    if (isTracking) {
        ...
        isTracking = false;
        geolocateButton.style.color = ""; // Reset icon color
    } else {
        map.addLayer(geolocateOverlay);
        const pos = geolocation.getPosition();
        if (pos) {
            map.getView().setCenter(pos);
            map.getView().setZoom(18); // Optional: Zoom in when activated
        }
        isTracking = true;
        geolocateButton.style.color = "#3399CC"; // Highlight button when active
    }
}
...
refreshButton.className = 'refresh-button fas fa-sync-alt'; // Uses FontAwesome icon
refreshButton.title = 'Actualizar Mapa';

const refreshControl = document.createElement('div');
refreshControl.className = 'ol-unselectable ol-control refresh-control';
refreshControl.appendChild(refreshButton);
map.getTargetElement().appendChild(refreshControl);

function handleRefresh() {
    // true forces the reload from the server, ignoring cache
...
//move controls inside containers, in order
    ...
    if (attributionControl) {
        ...
        //refresh
if (typeof refreshControl !== 'undefined') {
    topLeftContainerDiv.appendChild(refreshControl);
}
    }